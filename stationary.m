function volume = stationary(h, src, vent, showgr)
    if nargin < 1
        h = 0.65;
        showgr = true;
        src = 0.05;
    ...
    vent_str = vent; % 0.200-0.317 m3/s (0.786-2.49)
    ...
    %% Boundary Conditions
    edge_cond = @(location, state) abs(sum(evaluateGradient(flowresults, [location.x; location.y])));
